...
exports.getAll = (req, res) => {
    StoreSchema.findOne({solution: res.locals.user.solution, _id: req.params.store}, (err, store) => {
        if(err) return res.json({status: false, value: Strings.INVALID_STORE})

        SaleSchema.find({solution: res.locals.user.solution, store: store}, (err, sales) => {
            if(err)
